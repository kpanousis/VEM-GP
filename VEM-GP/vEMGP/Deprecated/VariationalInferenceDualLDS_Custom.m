function     [seq]= VariationalInferenceDualLDS_Custom(params,seq,optparams)
%VARIATIONALINFERENCEDUALLDS_CUSTOM Summary of this function goes here
%Konstantinos Panagiotis Panousis
%Gatsby Computational Neuroscience Unit
%University College London
%Code mostly from Lars Buesing VariationalInferenceDualLDS_Custom (2014)
...
for tr = 1:Trials
    ...
    times=seq(tr).times;
    clickTimes=seq(tr).clickTimes;
   
    VarInfparams.Q=condVarAndDerivs(hypers,times(2:end),times(1:end-1),clickTimes);
    T = size(seq(tr).y,2);
    
    VarInfparams.d          = repmat(params.model.d,T,1); if params.model.notes.useS; VarInfparams.d = VarInfparams.d + vec(params.model.W*seq(tr).s); end
    %Get mean from the GP
    %VarInfparams.mu=vec(meanAndDerivs(0,seq(tr).clickTimes,seq(tr).clickSigns,params.model.hypersPrior);
    
    %   VarInfparams.mu         =vec(meanAndDerivs(seq(tr).times',seq(tr).clickTimes,seq(tr).clickSigns,params.model.hypersPrior));
    VarInfparams.mu        = vec(getPriorMeanLDS_custom(VarInfparams,T,'seq',seq(tr)));
    VarInfparams.W          = Wmax(1:yDim*T,1:xDim*T);
    VarInfparams.y          = seq(tr).y;
    %Get Precision Matrix from the GP
    %VarInfparams.Lambda=sparse(kAISInverse2(seq(tr).times',seq(tr).clickTimes,params.model.hypersPrior));
    
    VarInfparams.Lambda    = buildPriorPrecisionMatrixFromLDS_Custom(VarInfparams,T);  % generate prior precision matrix
    ...
    lamOpt = minFunc(@VariationalInferenceDualCost_Custom,lamInit,optparams.minFuncOptions,VarInfparams);
    
    [DualCost, ~, varBound, m_ast, invV_ast, Vsm, VVsm, over_m, over_v] = VariationalInferenceDualCost_Custom(lamOpt,VarInfparams);
