function glmInitializationFor_glmBrodyLaplace(obj,modelNumber, sessionIdx, fittingParameters)
...
allRelevantSpikeInts = zeros(sum(relevantFrames(:,2) - relevantFrames(:,1) + 1), nNeurons);
%error('Have not yet implemented non-logical version of this code!')
previousFrames = 0;
for windowIdx = 1:size(relevantFrames,1)
    ...
    for relativeFrameIdx = 1:(relevantFrames(windowIdx,2) - relevantFrames(windowIdx,1) + 1)  %index within the window, as described by frame number
        ...
        if relativeFrameIdx == 1
            ...
            spikeIntHistory = zeros(relevantSelfSamples,nNeurons);
            ...
            for localSelfFrameNumber = 1:relevantSelfSamples
                ...
                for targetNeuronIdx = 1:nNeurons
                    if any(spikesInFrames{targetNeuronIdx} == globalSelfFrame)
                        ...
                        spikeIntHistory(localSelfFrameNumber,targetNeuronIdx) = sum(spikesInFrames{targetNeuronIdx} == globalSelfFrame);  %Now, set this equal to the number of spikes which fall in this frame, rather than just true
            ...
            %spikeBinHistory
            %spikeBinHistory = [spikeBinHistory(2:end,:);spikeStateNowBinary];
            spikeIntHistory = [spikeIntHistory(2:end,:);spikeStateNowInt];
        end
        
        
        %Create the self-history
        %spikeStateNowBinary = false(1,nNeurons);
        spikeStateNowInt = false(1,nNeurons);
        for targetNeuronIdx = 1:nNeurons
            if any(spikesInFrames{targetNeuronIdx} == globalFrameNumber)
                %spikeStateNowBinary(targetNeuronIdx) = true;
                spikeStateNowInt(targetNeuronIdx) = sum(spikesInFrames{targetNeuronIdx} == globalFrameNumber);
            end
        end
        ...
        transformedSpikingHistory = obj.modelGlobalParameters{modelNumber}.pseudoBasisSelf' * spikeIntHistory;  %nCompsPseudoBasis x nNeurons
        ...
        %allRelevantSpikeLogicals(previousFrames + relativeFrameIdx,:) = spikeStateNowBinary;
        allRelevantSpikeInts(previousFrames + relativeFrameIdx,:) = spikeStateNowInt;
...
for targetNeuronIdx = 1:nNeurons  
    ...
    if ~successfullyFitANeuronFlag
        ...
        if any(allRelevantSpikeInts(:,targetNeuronIdx)) && ~all(allRelevantSpikeInts(:,targetNeuronIdx))  %That is, the neuron fired a spike during the period of interest, and did not fire a spike in every window, such that we have something to fit, potentially
            %Create the rows of the corresponding matrices.
            [parameterizedWeights,FitInfo] = lassoglm(allRelevantInputs(:,relevantColumnsInFeatures),   allRelevantSpikeInts(:,targetNeuronIdx), 'poisson','Alpha',fittingParameters.alpha,'NumLambda',17);
        ...
        elseif ~any(allRelevantSpikeInts(:,targetNeuronIdx))
        ...
        [parameterizedWeights,FitInfo] = lassoglm(allRelevantInputs(:,relevantColumnsInFeatures),   allRelevantSpikeInts(:,targetNeuronIdx), 'poisson','Alpha',fittingParameters.alpha,'Lambda',lambdaValue);
