function [firingRates,varargout] = calculateFiringRates_BrodyGLM(obj,firing,latentTrajectory,startFrameNumber, modelNumber, sessionIdx,varargin)
%CALCULATEFIRINGRATES_BRODYGLM Returns the inferred firing rates (lambdas).
%calculateFiringRates_BrodyGLM.m
%Thomas Desautels
...
%Inputs: 
%   firing: (T_calculate + (startFrameNumber-1)) x nNeurons NON-BINARY
%       matrix of spiking activity.
...
assert(numel(obj.modelTypes) >= modelNumber  && any(strcmp(obj.modelTypes(modelNumber),{'glmBrodyLaplace','glmBrodyLaplaceSelfOnly','glmBrodyFixedSelfOnly'})), 'The model number given does not exist or is not a glmBrodyLaplace (or glmBrodyFixedSelfOnly) model.')

assert(~islogical(firing),'Firing is a logical matrix; this is the old case!')
...
if ~all(size(W) == [nNeurons , nNeurons * nCompsPseudoBasis])
    if all(size(W) == [nNeurons , nCompsPseudoBasis]) && any(strcmp(obj.modelTypes{modelNumber},{'glmBrodyLaplaceSelfOnly','glmBrodyFixedSelfOnly'}))  %If we're working with a self-weights only model and it's using the sparse representation:
