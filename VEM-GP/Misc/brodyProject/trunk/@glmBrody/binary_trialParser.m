function [spikingHistory,startFrameNumber,clickTimes,clickSigns] = binary_trialParser(obj,sessionIdx,trialIdx,bleedTimes,frameLength)
%BINARY_TRIALPARSER returns GLM-centric information on a designated trial
%binary_trialParser.m
%Thomas Desautels
...
%Outputs:
%       spikingHistory: BINARY spiking representation for all of the
%           neurons recorded: TxnNeurons.  May be reasonable to represent
%           as an uint8 in the future, for example.
...
fprintf('Binary (r180) version of trialParser.\n')
%% Input processing
...
%VECTOR
spikingHistory   = false(backwardBleedFrames + numFramesWithStimStartUntilIncludeTrialEnd,nNeurons);
...
for cellIdx = 1:nNeurons
    for spikeIdx = 1:numel(obj.trialData{sessionIdx}(trialIdx).spikes{cellIdx})
        ...
        if timePostStart >= -backwardBleedTime  && timePostStart <= timeFromStartToEnd
            ...
            appropriateTimeIndex = ceil(timePostStart / frameLength) + backwardBleedFrames;
            %Set the appropriate location in spikingHistory to true
            if ~spikingHistory(appropriateTimeIndex,cellIdx) 
            spikingHistory(appropriateTimeIndex,cellIdx) = true;
            else
            ...
            %For sanity checking:
            %fprintf(['Spike ',num2str(spikeIdx),' occurred at ',num2str(timePostStart),' in frame number ',num2str(appropriateTimeIndex),'.\n\n']);
        end
    end
end
...
if redundantSpikeCount > 0
    fprintf(['There were ',num2str(redundantSpikeCount),' spikes which fell in the same bin as a spike already\n',...
        'processed from the same cell! Since the spiking history is represented\n',...
        'by a binary vector, these spikes have been effectively lost!\n',...
        'Please consider using a shorter bin / frame length.\n\n'])
end
