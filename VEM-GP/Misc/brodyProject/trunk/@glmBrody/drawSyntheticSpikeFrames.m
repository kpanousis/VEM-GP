function generatedSpikesInFrames = drawSyntheticSpikeFrames(obj,synthesizingModelNum,sessionIdx,aOfTInGlobalFrameNumbers)
%DRAWSYNTHETICSPIKEFRAMES For a session, draw the frame numbers of spikes.
%
%Thomas Desautels
...
spikeHistory = zeros(relevantSelfSamples,nNeurons);
...
for neuronIdx = 1:nNeurons
    ...
    %Non-binary version:
    spikeHistory(:,neuronIdx) = poissrnd(ones(relevantSelfSamples,1) * exp(obj.modelSessionParameters{sessionIdx,synthesizingModelNum}.d(neuronIdx)));
end
...
for globalFrameNum = 1:numel(aOfTInGlobalFrameNumbers)
   ...
   transformedSpikingHistory = obj.modelGlobalParameters{synthesizingModelNum}.pseudoBasisSelf' * spikeHistory;  %now nBasisCompsSelf x nNeurons
   ...
   %New, non-binary version:
   spikeStateNow = initialDraw;
   
   
   for neuronIdx = 1:nNeurons
       if spikeStateNow(neuronIdx) > 0
           %generatedSpikesInFrames{neuronIdx} = vertcat(generatedSpikesInFrames{neuronIdx},globalFrameNum);
           generatedSpikesInFrames{neuronIdx} = vertcat(generatedSpikesInFrames{neuronIdx},globalFrameNum * ones(spikeStateNow(neuronIdx),1));
       end
   end
   ...
   spikeHistory = [spikeHistory(2:end,:);spikeStateNow]; 
